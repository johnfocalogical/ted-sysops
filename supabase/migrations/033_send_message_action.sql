-- ============================================================================
-- Migration: Add send_message action type to automator execution engine
-- Description: Extends execute_automator_step to handle 'send_message' actions.
--              Creates conversations if needed, inserts messages with
--              sender_type='automator', and records sent message IDs.
-- ============================================================================

-- ============================================================================
-- Update execute_automator_step to handle 'send_message' action type
-- ============================================================================
    ...
    LOOP
        ...
        BEGIN
            ...
            WHEN 'send_message' THEN
                ...
                v_msg_content := resolve_value_source(
                    v_action_params->'message_content', p_response
                );
                v_msg_target := v_action_params->>'target';
                v_automator_id := v_instance.automator_id;

                -- Determine target conversation
                CASE v_msg_target

                WHEN 'deal_chat' THEN
                    -- Find existing deal-linked conversation, or create one
                    SELECT cdl.conversation_id INTO v_msg_conversation_id
                    FROM conversation_deal_links cdl
                    WHERE cdl.deal_id = v_instance.deal_id
                    LIMIT 1;

                    IF v_msg_conversation_id IS NULL THEN
                        -- Create a new group conversation for the deal
                        INSERT INTO conversations (
                            team_id, type, name, created_by
                        ) VALUES (
                            v_instance.team_id, 'group', 'Deal Chat', p_user_id
                        )
                        RETURNING id INTO v_msg_conversation_id;

                        -- Add the executing user as participant
                        INSERT INTO conversation_participants (
                            conversation_id, user_id, role
                        ) VALUES (
                            v_msg_conversation_id, p_user_id, 'admin'
                        );

                        -- Link conversation to deal
                        INSERT INTO conversation_deal_links (
                            conversation_id, deal_id, linked_by
                        ) VALUES (
                            v_msg_conversation_id, v_instance.deal_id, p_user_id
                        );
                    END IF;

                WHEN 'channel' THEN
                    v_msg_conversation_id := (v_action_params->>'channel_id')::uuid;

                WHEN 'new_group' THEN
                    -- Create a new group conversation
                    -- Get deal address for conversation name
                    SELECT address INTO v_deal_address
                    FROM deals WHERE id = v_instance.deal_id;

                    INSERT INTO conversations (
                        team_id, type, name, created_by
                    ) VALUES (
                        v_instance.team_id, 'group',
                        COALESCE(v_deal_address, 'Deal') || ' - Auto Group',
                        p_user_id
                    )
                    RETURNING id INTO v_msg_conversation_id;

                    -- Add the executing user
                    INSERT INTO conversation_participants (
                        conversation_id, user_id, role
                    ) VALUES (v_msg_conversation_id, p_user_id, 'admin')
                    ON CONFLICT DO NOTHING;

                    -- Add explicit participant IDs
                    IF v_action_params->'participant_ids' IS NOT NULL THEN
                        INSERT INTO conversation_participants (conversation_id, user_id, role)
                        SELECT v_msg_conversation_id, (pid)::uuid, 'member'
                        FROM jsonb_array_elements_text(v_action_params->'participant_ids') AS pid
                        ON CONFLICT DO NOTHING;
                    END IF;

                    -- Add deal employees if requested
                    IF (v_action_params->>'include_deal_employees')::boolean IS TRUE THEN
                        INSERT INTO conversation_participants (conversation_id, user_id, role)
                        SELECT v_msg_conversation_id, de.user_id, 'member'
                        FROM deal_employees de
                        WHERE de.deal_id = v_instance.deal_id
                        ON CONFLICT DO NOTHING;
                    END IF;

                    -- Add deal owner if requested
                    IF (v_action_params->>'include_deal_owner')::boolean IS TRUE THEN
                        INSERT INTO conversation_participants (conversation_id, user_id, role)
                        SELECT v_msg_conversation_id, d.created_by, 'member'
                        FROM deals d
                        WHERE d.id = v_instance.deal_id
                        ON CONFLICT DO NOTHING;
                    END IF;
                ...
                ELSE
                    RAISE EXCEPTION 'Unknown send_message target: %', v_msg_target;

                END CASE;

                -- Insert the message with automator sender type
                IF v_msg_conversation_id IS NOT NULL AND v_msg_content IS NOT NULL THEN
                    INSERT INTO messages (
                        conversation_id, sender_id, sender_type,
                        sender_automator_id, content, metadata
                    ) VALUES (
                        v_msg_conversation_id, NULL, 'automator',
                        v_automator_id, v_msg_content,
                        CASE WHEN (v_action_params->>'include_deal_link')::boolean IS TRUE THEN
                            jsonb_build_object(
                                'deal_references', jsonb_build_array(
                                    jsonb_build_object(
                                        'deal_id', v_instance.deal_id
                                    )
                                )
                            )
                        ELSE '{}'::jsonb
                    ...
                    WHERE id = v_msg_conversation_id;

                    v_action_result := v_action_result || jsonb_build_object(
                        'details', jsonb_build_object(
                            'message_id', v_msg_id,
                            'conversation_id', v_msg_conversation_id
                        )
                    );
                ELSE
                    v_action_result := jsonb_build_object(
                        'action_type', v_action_type,
                        'success', false,
                        'error', 'No conversation or message content resolved'
                    );
                END IF;
