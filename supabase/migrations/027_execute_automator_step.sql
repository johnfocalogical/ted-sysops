-- ============================================================================
-- Migration: Automator Execution Engine Functions
-- Description: Core Postgres functions for starting and executing automator
--              instances. These handle step validation, recording, backend
--              action execution, and next-node resolution transactionally.
-- ============================================================================

-- ============================================================================
-- 1. start_automator_instance
-- Starts a new automator instance on a deal, snapshots the definition,
-- auto-completes the Start node, and returns the first interactive node.
-- ============================================================================
...
BEGIN
    ...
    -- Find the first interactive node definition
...
-- ============================================================================
-- 2. execute_automator_step
-- Core step execution: validates, records, executes backend actions,
-- resolves next node, all within a single transaction.
-- ============================================================================
...
-- ============================================================================
-- 3. resolve_value_source (helper)
-- Resolves a ValueSource JSONB to its actual value at execution time.
-- ============================================================================

CREATE OR REPLACE FUNCTION resolve_value_source(
    p_value_source JSONB,
    p_response JSONB DEFAULT NULL
)
RETURNS TEXT
LANGUAGE plpgsql
...
DECLARE
    v_source TEXT;
BEGIN
    IF p_value_source IS NULL THEN
        RETURN NULL;
    END IF;

    v_source := p_value_source->>'source';

    CASE v_source
        WHEN 'static' THEN
            RETURN p_value_source->>'value';
        WHEN 'today' THEN
        ...
        WHEN 'field' THEN
            IF p_response IS NULL THEN
                RETURN NULL;
            END IF;
            RETURN p_response->>( p_value_source->>'field_id' );
        ELSE
            RETURN NULL;
    END CASE;
END;
