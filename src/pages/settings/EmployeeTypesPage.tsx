...
import { getTeamEmployeeTypes } from '@/lib/teamTypeService'
import type { TeamEmployeeTypeWithUsage } from '@/types/type-system.types'
...
export function EmployeeTypesPage() {
  ...
  const [types, setTypes] = useState<TeamEmployeeTypeWithUsage[]>([])
  const [loading, setLoading] = useState(true)
  const [showWizard, setShowWizard] = useState(false)
  const [editingType, setEditingType] = useState<TeamEmployeeTypeWithUsage | null>(null)
  const [manageFieldsType, setManageFieldsType] = useState<TeamEmployeeTypeWithUsage | null>(null)
  ...
  const loadTypes = async () => {
    ...
    setLoading(true)
    try {
      const data = await getTeamEmployeeTypes(context.team.id)
      setTypes(data)
    } catch (err) {
    ...
    <div className="max-w-4xl">
      ...
      <div className="mb-6">
        ...
        <p className="text-muted-foreground mt-1">
          Manage employee type categories and their custom fields
        </p>
      </div>
        ...
        <CardContent>
          {loading ? (
            ...
            <TypeList
              types={types}
              entityType="employee"
              onEdit={(type) => setEditingType(type as TeamEmployeeTypeWithUsage)}
              onManageFields={(type) => setManageFieldsType(type as TeamEmployeeTypeWithUsage)}
              onRefresh={loadTypes}
            />
          )}
        </CardContent>
