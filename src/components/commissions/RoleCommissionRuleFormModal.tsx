...
import {
  createRoleCommissionRule,
  updateRoleCommissionRule,
} from '@/lib/roleCommissionRuleService'
import { validateCommissionConfiguration } from '@/lib/commissionRuleService'
...
// ============================================================================
// Schema (no date range fields for role-level rules)
// ============================================================================
...
interface RoleCommissionRuleFormModalProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  roleId: string
  teamId: string
  rule?: RoleCommissionRuleWithCreator | null
  userId: string
  onSaved: () => void
}
...
export function RoleCommissionRuleFormModal({
  open,
  onOpenChange,
  roleId,
  teamId,
  ...
  userId,
  onSaved,
}: RoleCommissionRuleFormModalProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      priority: 0,
      notes: '',
      config_amount: undefined,
      config_minimum_deal_profit: undefined,
      config_percentage: undefined,
      config_cap: undefined,
      config_profit_basis: 'gross',
      config_base_type: 'percentage_gross',
      config_base_percentage: undefined,
    },
  })
  ...
  useEffect(() => {
    ...
    if (rule) {
      ...
      switch (rule.calculation_type) {
        ...
        case 'role_based': {
          ...
          if (mults && Object.keys(mults).length > 0) {
            setRoleMultipliers(
              Object.entries(mults).map(([role, multiplier]) => ({ role, multiplier }))
            )
          } else {
      ...
      setTiers([{ threshold: 0, percentage: 10 }])
      setRoleMultipliers([{ role: '', multiplier: 1 }])
    }
  }, [open, rule, form])
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (isEditing && rule) {
        await updateRoleCommissionRule(rule.id, {
          name: values.name,
        ...
        await createRoleCommissionRule(
          {
            team_id: teamId,
            role_id: roleId,
            name: values.name,
      ...
      console.error('Error saving role commission rule:', err)
      toast.error(err instanceof Error ? err.message : 'Failed to save commission rule')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {isEditing ? 'Edit Role Commission Rule' : 'Add Role Commission Rule'}
          </DialogTitle>
          <DialogDescription>
            {isEditing
              ? 'Update the commission rule for this role'
              : 'Define a commission rule that applies to all employees with this role'}
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="e.g., Standard Deal Commission" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            {calculationType === 'flat_fee' && (
              <div className="space-y-4 rounded-lg border p-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>Fixed fee per qualifying deal</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                <FormField
                  control={form.control}
                  name="config_minimum_deal_profit"
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>Only applies if deal profit meets this threshold (optional)</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>Maximum commission amount per deal (optional)</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                ...
                <div className="space-y-2">
                  ...
                  {tiers.map((tier, index) => (
                    <div key={index} className="flex items-center gap-2">
                      ...
                      {tiers.length > 1 && (
                        <Button type="button" variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive shrink-0" onClick={() => removeTier(index)}>
                  ...
                  {roleMultipliers.map((rm, index) => (
                    <div key={index} className="flex items-center gap-2">
                      ...
                      {roleMultipliers.length > 1 && (
                        <Button type="button" variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive shrink-0" onClick={() => removeRoleMultiplier(index)}>
                          <Trash2 className="h-4 w-4" />
                        </Button>
                      )}
                    </div>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>Higher priority rules are evaluated first (default: 0)</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="Optional notes about this rule" rows={2} className="resize-none" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
