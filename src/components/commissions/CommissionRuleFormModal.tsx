...
interface CommissionRuleFormModalProps {
  ...
  teamId: string
  rule?: CommissionRuleWithCreator | null
  userId: string
  onSaved: () => void
}

// ============================================================================
// Helper: Build configuration from form values + local state arrays
// ============================================================================
...
export function CommissionRuleFormModal({
  ...
  // Local state for dynamic arrays (tiers & role multipliers)
  ...
  const addRoleMultiplier = () =>
    setRoleMultipliers((prev) => [...prev, { role: '', multiplier: 1 }])
  const removeRoleMultiplier = (index: number) =>
    setRoleMultipliers((prev) => prev.filter((_, i) => i !== index))
  const updateRoleMultiplier = (
    index: number,
    field: 'role' | 'multiplier',
    value: string | number
  ) =>
    setRoleMultipliers((prev) =>
      prev.map((rm, i) => (i === index ? { ...rm, [field]: value } : rm))
    )
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {isEditing ? 'Edit Commission Rule' : 'Add Commission Rule'}
          </DialogTitle>
          <DialogDescription>
            {isEditing
              ? 'Update the commission rule configuration'
              : 'Define a new commission rule for this employee'}
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="e.g., Standard Acquisition Fee" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            {/* ============================================================ */}
            {/* Dynamic config section based on calculation_type */}
            {/* ============================================================ */}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        Only applies if deal profit meets this threshold (optional)
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
            ...
            {(calculationType === 'percentage_gross' ||
              calculationType === 'percentage_net') && (
              <div className="space-y-4 rounded-lg border p-4">
                <div className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                  {calculationType === 'percentage_gross'
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        Maximum commission amount per deal (optional)
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
              ...
              <div className="space-y-4 rounded-lg border p-4">
                ...
                <div className="space-y-2">
                    ...
                    <div key={index} className="flex items-center gap-2">
                      <div className="relative flex-1">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            updateTier(index, 'threshold', Number(e.target.value))
                          }
                        />
                      </div>
                      <div className="relative w-24">
                        <Input
                          ...
                          onChange={(e) =>
                            updateTier(index, 'percentage', Number(e.target.value))
                          }
                        />
                        <span className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground text-sm">
                          %
                  ...
                  <p className="text-xs text-muted-foreground">
                    Thresholds should be in ascending order. The last tier applies to all
                    amounts above its threshold.
                  </p>
                </div>
              </div>
            )}
            ...
            {calculationType === 'role_based' && (
              <div className="space-y-4 rounded-lg border p-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <Select
                        value={field.value || 'percentage_gross'}
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="percentage_gross">
                            % of Gross Profit
                          </SelectItem>
                          <SelectItem value="percentage_net">
                            % of Net Profit
                          </SelectItem>
                        </SelectContent>
                      </Select>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        Base percentage before role multiplier is applied
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <div className="space-y-2">
                  ...
                  {roleMultipliers.map((rm, index) => (
                    <div key={index} className="flex items-center gap-2">
                      <Input
                        ...
                        value={rm.role}
                        onChange={(e) =>
                          updateRoleMultiplier(index, 'role', e.target.value)
                        }
                      />
                      <div className="relative w-24">
                        <Input
                          ...
                          value={rm.multiplier}
                          onChange={(e) =>
                            updateRoleMultiplier(index, 'multiplier', Number(e.target.value))
                          }
                        />
                        <span className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground text-sm">
                          ×
                  ...
                  <p className="text-xs text-muted-foreground">
                    The final commission = base percentage × role multiplier
                  </p>
                </div>
              </div>
            )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>Leave blank for ongoing</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
